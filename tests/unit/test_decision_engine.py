...
class TestFraudDecisionEngine:
    def setup_method(self):
        self.engine = FraudDecisionEngine(ThresholdConfig(approve_below=0.60, review_above=0.75, block_above=0.90))
    ...
    def test_override_blacklisted_device(self):
        decision, level, overrides = self.engine.evaluate(0.1, OverrideFlags(blacklisted_device=True))
        assert decision == Decision.BLOCK
        assert level == RiskLevel.CRITICAL
        assert "BLACKLISTED_DEVICE" in overrides

    def test_override_sanctioned_country(self):
        decision, _, overrides = self.engine.evaluate(0.05, OverrideFlags(sanctioned_country=True))
        assert decision == Decision.BLOCK
        assert "SANCTIONED_COUNTRY" in overrides

    def test_override_blacklisted_ip(self):
        decision, _, overrides = self.engine.evaluate(0.2, OverrideFlags(blacklisted_ip=True))
        assert decision == Decision.BLOCK
        assert "BLACKLISTED_IP" in overrides

    def test_override_velocity_anomaly_elevates(self):
        decision, _, overrides = self.engine.evaluate(0.5, OverrideFlags(velocity_anomaly=True))
        assert decision == Decision.REVIEW
        assert "VELOCITY_ANOMALY" in overrides

    def test_override_velocity_no_effect_on_high_score(self):
        decision, _, _ = self.engine.evaluate(0.85, OverrideFlags(velocity_anomaly=True))
...
class TestAMLDecisionEngine:
    def setup_method(self):
        self.engine = AMLDecisionEngine(AMLThresholdConfig(medium_above=0.50, high_above=0.70, critical_above=0.85))
