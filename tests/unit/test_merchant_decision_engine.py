...
class TestMerchantDecisionEngine:
    ...
    def test_chargeback_rate_override_blocks(self):
        decision, level, tier, overrides = self.engine.evaluate(
            0.2, chargeback_rate=0.12
        )
        assert decision == Decision.BLOCK
        assert tier == "BLOCKED"
    ...
    def test_confidence_aware_cold_maturity(self):
        ...
        decision, _, tier, _ = self.engine.evaluate(
            0.35, confidence=0.2, maturity=MaturityLevel.COLD
        )
    ...
    def test_confidence_aware_hot_maturity(self):
        ...
        decision, _, tier, _ = self.engine.evaluate(
            0.35, confidence=0.95, maturity=MaturityLevel.HOT
        )
        assert decision == Decision.APPROVE
        assert tier == "STANDARD"
